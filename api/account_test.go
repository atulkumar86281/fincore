...
func TestGetAccountApi(t *testing.T){
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationHeaderType, account.Owner, time.Minute)
			},
			buildStubs: func(store *mockDb.MockStore) {
				store.EXPECT().
		...
		},
		{
			...
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationHeaderType, "unauthorized_user", time.Minute)
			},
			buildStubs: func(store *mockDb.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account.ID)).
...
func TestListAccountsAPI(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		accounts[i] = randomAccount(account.Owner)
	}
